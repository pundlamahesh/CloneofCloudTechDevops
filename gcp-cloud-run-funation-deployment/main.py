...
def get_layout(title, content):
    """
    Wraps content in the HTML structure.
    NOTE: We use query parameters (?page=...) for navigation links 
    because Cloud Functions run on a single URL endpoint.
    """
    return f"""
    <html>
    <head>
    ...
    <body>
        ...
        <div class="navbar">
            ...
            <a href="?page=syllabus">Syllabus</a>
        </div>
...
# --- 2. Content Generators (Full Data) ---
...
# --- 3. The Main Cloud Function Entry Point ---

@functions_framework.http
def hello_http(request):
    """
    HTTP Cloud Function.
    Decides whether to show the Home Page or Syllabus based on the '?page=' URL parameter.
    """
    # 1. Capture the query parameter 'page' (e.g. url?page=syllabus)
    #    Default to 'home' if no parameter is provided.
    request_args = request.args
    page = request_args.get('page', 'home')

    # 2. Logic to switch content
    if page == 'syllabus':
        content = get_syllabus_content()
        title = "Syllabus - Veera Sir"
    else:
        content = get_home_content()
        title = "Home - Naresh IT DevOps"

    # 3. Return the full HTML
    return get_layout(title, content)
